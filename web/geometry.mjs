...
export function validate(input){
 ...
 for(const k of ['rows','columns','series','stringsPerInverter','invertersPerBlock'])if(!Number.isInteger(s[k]))throw Error(k+' must be a whole number');
...
export function array(supplied){
 ...
 const directions=s.layout==='east-west'?[s.azimuth,(s.azimuth+180)%360]:[s.azimuth];const modules=[];
 ...
 return {modules,span,run,rise:run*Math.sin(rad(s.tilt)),projectedRun:run*Math.cos(rad(s.tilt)),height:s.height,settings:s};
}
